...
(function () {
    ...
    function writeToXterm(line) {
        try {
            xtermBuffer.push(line);
            if (xtermBuffer.length > 2000) xtermBuffer.shift();
            if (xtermInstance) xtermInstance.writeln(line);
        } catch (e) { /* 忽略 */ }
    }
    ...
    window.queryIpPort = async function() {
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                const _ST_MAP = {survival:"生存", creative:"创造", adventure:"冒险", skyblock:"空岛", minigame:"小游戏", pvp:"PVP", peaceful:"和平", hardcore:"极限", anarchy:"无政府", factions:"阵营", prison:"监狱", bedwars:"起床战争", skywars:"空岛战争", hunger:"饥饿游戏", redstone:"红石", kitpvp:"装备PVP", lobby:"大厅", practice:"练习"};
                if (d.server_type) html += `<div class="ipq-row"><span class="ipq-label"><i class="fas fa-tag"></i> 服务器类型</span><span>${escapeHtml(String(_ST_MAP[d.server_type] || d.server_type))}</span></div>`;
                ...
                if (d.world_id) html += `<div class="ipq-row"><span class="ipq-label" title="网易租赁服世界的唯一标识"><i class="fas fa-map"></i> 世界ID</span><span class="mono">${escapeHtml(String(d.world_id))}</span></div>`;
